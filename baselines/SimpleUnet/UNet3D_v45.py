...
from .dataloader_v45 import CogDataset_v45
...
# Define arguments
parser.add_argument("--runname", type=str, default="Cond3D_v45", help="The model name on the HF Hub")
...
parser.add_argument("--num_epochs", type=int, default=50, help="The number of training epochs")
...
parser.add_argument("--in_channel", type=int, default=12, help="number of input channel")
parser.add_argument("--out_channel", type=int, default=3, help="number of output channel")
parser.add_argument("--time_span", type=int, default=9, help="The number of frame")

parser.add_argument("--DLAcc", type=int, default=1, help="Use accelerator for dataloader")


# cross_attention_dim
parser.add_argument("--cross_attention_dim", type=int, default=1024, help="The cross attention dim")
parser.add_argument("--MaskedInput", type=int, default=0, help="0 for no masked input, 1 for masked input")
...
parser.add_argument("--loss_threshold", type=float, default=1.0, help="The loss threshold")
...
args.runname += f"_0426_I{args.in_channel}O{args.out_channel}T{args.time_span}-Blc{args.model_blocks}"
if args.model_type != "UNet3D_src":
    args.runname += f"-OriginalUNet3D"
args.runname += f"-LR{args.lr}-LPB{args.LPB}"
if args.MaskedInput != 0:
    args.runname += f"-MaskedInput{args.MaskedInput}"
...
if args.num_workers != 2:
    args.runname += f"-W{args.num_workers}"
if args.loss_threshold != 1.0:
    args.runname += f"-LossT{args.loss_threshold}"
if args.grad_accm != 1:
    args.runname += f"-GradAcc{args.grad_accm}"
if args.image_size != 256:
    args.runname += f"-Size{args.image_size}"
...
train_dataset = CogDataset_v45(num_frames=args.time_span, image_size=args.image_size, mode=args.mode)
...
elif args.model_type == "UNet3D_src":
...
model = UNet3DConditionModel(
    ...
    block_out_channels=(128, 128, 256, args.max_dim, args.max_dim, args.max_dim, args.max_dim)[:len(args.model_blocks)],  # the number of output channels for each UNet block
    down_block_types=down_block_list,  # the down block sequence
    up_block_types=up_block_list,  # the up block sequence
    cross_attention_dim=args.cross_attention_dim,  # the dimension of the cross attention
...
def visualization_v44(args, x1, p1, c1, x2, p2, scale=0.3, auto=False):
    ...
    for batch_idx in range(x1.size(0)):
        ...
        fig, axs = plt.subplots(6, args.time_span, figsize=(args.time_span*2, 12))

        if auto:
            temp = np.percentile(x1[batch_idx,:3].cpu().numpy(), 98, axis=(0,2,3))
            temp = np.percentile(temp, 20)
            scale = min(temp, scale)
            print(f"Scale: {scale}")
        
        for frame_idx in range(args.time_span):
            
            img = pp(x1[batch_idx, 0:3, frame_idx])
            img = pp2(img, scale=scale)
            axs[0, frame_idx].imshow(img)
            if frame_idx == 0: axs[0, frame_idx].set_ylabel("Input \n MSI", size=14)
            
            img = pp(x1[batch_idx, 9:12, frame_idx])
            img = pp2(img, 1)
            img[:,:,0] = img[:,:,1]
            axs[1, frame_idx].imshow(img)
            if frame_idx == 0: axs[1, frame_idx].set_ylabel("Input \n SAR", size=14)

            img = pp(p1[batch_idx, 0:3, frame_idx])
            img = pp2(img, scale=scale)
...
class SquareMaskGenerator(nn.Module):
    ...
    def forward(self):
        ...
        with torch.no_grad():
            ...
            mask = (torch.rand(self.shape) < threshold / (mask_scale*2) ** 2).float().to(self.device)
            mask = F.max_pool2d(mask, mask_scale+1, stride=1, padding=int((mask_scale+1)/2))
            mask = mask.unsqueeze(1).expand(-1, 10, -1, -1, -1)
        return mask
...
class NoisyProcess(nn.Module):
    ...
    def forward(self, clean_imgs):
        ...
        # cloud
        real_cloud = clean_imgs[:, 12:13]
        cloud_mask_synthesis = self.sauare_mask_generator().to(self.device) * random_scale(args) * 0.5
        ...
        threshold = 0.3
        clean_imgs_cloud_mask = (torch.clip(raw_data[:,12:13], threshold, 1) - threshold) / (1 - threshold)
        clean_imgs_cloud = clean_imgs[:, :10] * clean_imgs_cloud_mask
        ...
        # shadow
        real_shadow = clean_imgs[:, 12:13]
        shadow_mask_synthesis = self.sauare_mask_generator().to(self.device) * random_scale(args) * 0.5
        ...
        noisy_imgs[:, :10] *= (1 - shadow_mask * (random_scale(args) * 0.5 + 0.4))
        noisy_imgs[:, :10] = noisy_imgs[:, :10] * (1-interpolation_mask) + perm_clean_imgs_cloud[:, :10] * interpolation_mask
        noisy_imgs[:, :10] += cloud_mask 
        ...
        if batch_temporal_cloud_mask.sum() < args.time_span // 2:
            print("Masking half of the images")
            random_batch_temporal_cloud_mask = (torch.rand([args.train_bs, 1, args.time_span, 1, 1]).to(args.device) > 0.75) * 1.
            noisy_imgs[:,:10] = noisy_imgs[:,:10] * (1 - random_batch_temporal_cloud_mask) + random_batch_temporal_cloud_mask * random_scale(args) * 2

        return noisy_imgs

def random_scale_with_channel(args):
    return torch.rand([args.train_bs, 10, args.time_span, 1, 1]).to(args.device)
...
for args.epoch in range(args.num_epochs):
    ...
    for args.step, (raw_data, meta_info, daycounts, dates) in enumerate(train_dataloader):
        ...
        with torch.no_grad():
            ...
            cloud_mask = cloud_process(raw_data[:, 13:15].sum(dim=1, keepdim=False) >= 1)
            loss_mask = 1 - cloud_mask.unsqueeze(1)
            clean_imgs = raw_data[:, :12]
            noisy_imgs = noisy_process(raw_data)[:, :12]
            ...
            if args.step % 100 == 0:
                ...
                with torch.no_grad():
                    with accelerator.accumulate(model):
                        ...
                        # visualization_v44(args, clean_imgs, clean_pred, loss_mask, noisy_imgs, noisy_pred, scale=1.0, auto=False)
        ...
        with accelerator.accumulate(model):
            ...
            # Full range loss
            loss1 = (F.mse_loss(pred, clean_imgs_[:, :args.out_channel], reduction="none") * loss_mask).mean() * 3
