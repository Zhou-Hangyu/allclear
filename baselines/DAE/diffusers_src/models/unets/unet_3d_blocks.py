...
def get_down_block(
    ...
    "DownBlock3D",
    "DownBlockJust2D",  # added for SDV
    "CrossAttnDownBlock3D",
    "CrossAttnDownBlock2D1D",
    "DownBlockMotion",
    ...
    elif down_block_type == "DownBlockJust2D":
        return DownBlockJust2D(
            num_layers=num_layers,
            in_channels=in_channels,
            out_channels=out_channels,
            temb_channels=temb_channels,
    ...
    elif down_block_type == "CrossAttnDownBlock2D1D":
        if cross_attention_dim is None:
            raise ValueError("cross_attention_dim must be specified for CrossAttnDownBlock3D")
        return CrossAttnDownBlock2D1D(
            num_layers=num_layers,
            in_channels=in_channels,
            out_channels=out_channels,
            temb_channels=temb_channels,
...
def get_up_block(
    ...
    elif up_block_type == "UpBlockJust2D":
        return UpBlockJust2D(
            num_layers=num_layers,
            in_channels=in_channels,
            out_channels=out_channels,
            prev_output_channel=prev_output_channel,
    ...
    elif up_block_type == "CrossAttnUpBlock2D1D":
        if cross_attention_dim is None:
            raise ValueError("cross_attention_dim must be specified for CrossAttnUpBlock3D")
        return CrossAttnUpBlock2D1D(
            num_layers=num_layers,
            in_channels=in_channels,
            out_channels=out_channels,
            prev_output_channel=prev_output_channel,
...
class UNetMidBlock3DCrossAttn(nn.Module):
    def __init__(
        ...
        logger.info(f"UNetMidBlock3DCrossAttn: Use only four simple modules")
    ...
    def forward(
        ...
        for attn, temp_attn, resnet, temp_conv in zip(
            self.attentions, self.temp_attentions, self.resnets, self.temp_convs
        ):
            hidden_states = attn(
                hidden_states,
                encoder_hidden_states=encoder_hidden_states,
...
class CrossAttnDownBlock2D1D(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
    ...
    def forward(
        ...
        for resnet, temp_attn in zip(
            self.resnets, self.temp_attentions
        ):
            hidden_states = resnet(hidden_states, temb)
            hidden_states = temp_attn(
                hidden_states,
...
class DownBlockJust2D(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class CrossAttnUpBlock2D1D(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class UpBlockJust2D(nn.Module):
    def __init__(
        ...
        num_layers = 2
        print("Using UpBlockJust2D with 2 ResnetBlocks")
        logger.info(f"Using UpBlockJust2D with 2 ResnetBlocks")
        for i in range(num_layers):
            res_skip_channels = in_channels if (i == num_layers - 1) else out_channels
            resnet_in_channels = prev_output_channel if i == 0 else out_channels
