...
class TemporalConvLayer(nn.Module):
    ...
    def __init__(
        ...
        # conv layers
        # print("Use three conv layers in TemporalConvLayer")
        # logger.info("Use three conv layers in TemporalConvLayer")
        self.conv1 = nn.Sequential(
            nn.GroupNorm(norm_num_groups, in_dim),
            nn.SiLU(),
            nn.Conv3d(in_dim, hid_dim, (5, 1, 1), padding=(2, 0, 0)),
        )
        self.conv2 = nn.Sequential(
            nn.GroupNorm(norm_num_groups, hid_dim),
            nn.SiLU(),
            nn.Dropout(dropout),
            nn.Conv3d(hid_dim, in_dim, (3, 1, 1), padding=(1, 0, 0)),
        )
        self.conv3 = nn.Sequential(
            nn.GroupNorm(norm_num_groups, in_dim),
            nn.SiLU(),
            nn.Dropout(dropout),
            nn.Conv3d(in_dim, hid_dim, (3, 1, 1), padding=(1, 0, 0)),
        )
        self.conv4 = nn.Sequential(
            nn.GroupNorm(norm_num_groups, hid_dim),
            nn.SiLU(),
            nn.Dropout(dropout),
            nn.Conv3d(hid_dim, out_dim, (3, 1, 1), padding=(1, 0, 0)),
        )
