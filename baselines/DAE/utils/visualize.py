...
def visualization_v44(step, args, x1, p1, c1, x2, p2, daycounts, scale=0.3, auto=False):
    x1 = x1.cpu()
    p1 = p1.cpu()
    c1 = c1.cpu()
    x2 = x2.cpu()
    ...
    for batch_idx in range(x1.size(0)):
        ...
        fig, axs = plt.subplots(6, args.tx, figsize=(args.tx * 2, 12))
        ...
        for frame_idx in range(args.tx):
            print(batch_idx, frame_idx, x1.shape, args.tx)
        ...
        fig.tight_layout()
        plt.savefig(
            os.path.join(args.output_dir, f"EP{args.epoch}_S{step}_B{batch_idx}_Scale{int(scale * 100)}.png"))
        plt.pause(0.2)
        plt.close()