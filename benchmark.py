    ...
    def setup_data_loader(self):
        ...
        if self.args.dataset_type == "AllClear":
            ...
            dataset = CRDataset(
                ...
                stp_mode=self.args.uc_stp_mode,
            )
    ...
    def run(self):
        ...
        for eval_iter, data in tqdm(enumerate(self.data_loader), total=len(self.data_loader), desc="Evaluating Batches"):
            ...
            with torch.no_grad():
                ...
                if self.args.jitter_noise == "none":
                    target_cld_shdw_mask = (data["target_cld_shdw"][:,0,...] + data["target_cld_shdw"][:,1,...]) > 0
                # if jitter_noise = "fp", add additional 10% noise (via uniform prob < 0.1) to the target_cld_shdw_mask
                elif self.args.jitter_noise == "fp":
                    # set seed 
                    torch.manual_seed(eval_iter+self.args.jitter_noise_seed)
                    target_cld_shdw_mask = (data["target_cld_shdw"][:,0,...] + data["target_cld_shdw"][:,1,...] + torch.rand_like(data["target_cld_shdw"][:,0,...]) < self.args.jitter_noise_scale) > 0
                # if jitter_noise = "fn", set target_cld_shdw_mask to be 0
                elif self.args.jitter_noise == "fn":
                    target_cld_shdw_mask = torch.zeros_like(target_cld_shdw_mask)
            ...
            lulc_maps.append(batch_lulc_maps)
            target_cld_shdw_masks.append(batch_masks)
            if self.args.dataset_type == "AllClear":
                batch_lulc_maps = torch.cat(target_lulc_maps, dim=0)
        ...
        torch.save(targets, f"{output_path}/{self.args.model_name}_targets.pt")
        lulc_maps = torch.cat(lulc_maps, dim=0).cpu()
        torch.save(lulc_maps, f"{output_path}/{self.args.model_name}_lulc_maps.pt")
        target_cld_shdw_masks = torch.cat(target_cld_shdw_masks, dim=0).cpu()
        torch.save(target_cld_shdw_masks, f"{output_path}/{self.args.model_name}_target_cld_shdw_masks.pt")
...
def parse_arguments():
    ...
    parser.add_argument("--stp-mode", type=str, default="allfillones", choices=["allfillones", "allfillzeros", "s1_available_mask", "s1fillzeros"])
    ...
    uc_args.add_argument("--uc-stp-mode", type=str, default="allfillones")
    ...
    parser.add_argument("--jitter_noise", type=str, default="none", choices=["fp", "fn", "none"], help="Experiment name")
    parser.add_argument("--jitter_noise_scale", type=float, default=0.1, help="")
    parser.add_argument("--jitter_noise_seed", type=float, default=0, help="")
...
if __name__ == "__main__":
    ...
    print(f"STP Mode: {args.uc_stp_mode}")
...
#             print(f"main sensor: {self.args.main_sensor}") 
#             print(f"aux sensors: {self.args.aux_sensors}")
#             print(f"aux data: {self.args.aux_data}")
#             dataset = CRDataset(
#                 dataset=dataset,
#                 selected_rois=selected_rois,
#                 main_sensor=self.args.main_sensor,
...
#         if self.args.sen12mscrts_reset_datetime == "zeros":
#             dates = torch.zeros_like(dates)
#         elif self.args.sen12mscrts_reset_datetime == "min":
...
#             # if self.args.eval_iter % 2 == 0:
#             #     # break
#             #     continue

#             # if self.args.eval_iter == 100:
#             #     break
