        ...
        if mode == 's2p':
            ...
            for df_idx in range(N - tx):
                ...
                for sensor_name, sensor_df in sensors.items():
                    ...
                    if sensor_name == "s1" and sensor_roi_df.shape[0] == 0:
                        break
                    # if not enough images in the time series, resample some of them to match the number of images in the main sensor
                    if sensor_roi_df.shape[0] < tx:
                        sensor_roi_df = sensor_roi_df.sample(tx, replace=True)
                    entry[sensor_name] = [
                        (date.strftime('%Y-%m-%d %H:%M:%S'), path)
                        for date, path in zip(sensor_roi_df['capture_date'], sensor_roi_df['image_file_path'])
                    ]
                    # sort the images by date
                    entry[sensor_name] = sorted(entry[sensor_name], key=lambda x: x[0])
